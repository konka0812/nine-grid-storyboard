...
4. 输出完整视频提示词（默认 H3 三字段版 + 中文通用版，对白用 `<d>[Chinese]</d>` 逐字内联并保持中文；双图提交时必须包含首帧锚定块）。
5. 用户确认后再交给可用视频工具。
...
12. 视频提示词默认输出 H3 三字段版，并附中文通用版；对白两版都保持中文。
13. 带对白时：说话人 ID 稳定并附声音画像，台词逐字原文落在对应镜头内；跨切用 `<scenetrans>`，截断用 `<cutoff>`，画外音声明嘴唇闭合。
...
5. 每个可提交片段都有 H3 三字段版和中文通用版各一条完整视频提示词（对白保持中文）。
